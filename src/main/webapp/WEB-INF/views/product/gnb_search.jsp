		...
		<ul>
			<li>
				<c:if test="${paging.startPage!=1 }"> 
	      		  	<a href="gnb_search?nowPage=${paging.startPage-1 }&cntPerPage=${paging.cntPerPage}&sname=${sname}">◀</a> 
	   			</c:if>  
	            ...
	            <c:when test="${p != paging.nowPage }"> 
	               <a href="gnb_search?nowPage=${p}&cntPerPage=${paging.cntPerPage}&sname=${sname}">${p}</a>
	            </c:when>   
		         </c:choose>
		      </c:forEach>
			      <c:if test="${paging.endPage != paging.lastPage}">
			      <a href="gnb_search?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}&sname=${sname}">▶</a>
			   </c:if>
			<li>   
		</ul>
	</div>
